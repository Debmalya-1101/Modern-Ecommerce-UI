...
Integrating a third-party script like Razorpay into an Angular application requires handling external JavaScript libraries dynamically.

Backend Analogy:
- Similar to how a backend application might use an HTTP client to communicate with an external API (like Stripe or PayPal), the frontend needs to load the Razorpay checkout script and interact with it.
...
const rzp = new Razorpay({
  key: 'rzp_test_...', // Public key from environment
  amount: 2000,
  order_id: 'order_123',
  handler: (response: any) => {
    // Send response.razorpay_signature to backend for verification
  }
});
rzp.open();
```
...
What this means:
- We declare `Razorpay` so TypeScript doesn't complain about the global variable.
- We configure the checkout with our `key` (public) and `order_id` (generated by our secure backend).
- The `handler` callback receives the payment success details.

Security Concept:
- **Public Key**: Placed in `environment.development.ts`. Safe to expose to the browser.
- **Secret Key**: Must **never** be in the Angular app. It lives on the backend to verify the signature returned in the `handler`.

### 17. State Sharing in Router Navigation

In frontend routing, when navigating between pages (like from a checkout or payment processing screen to a status page), you often want to pass data along without exposing it in the URL query parameters (which users can edit or bookmark).

#### How it works:
Angular's router allows passing arbitrary state objects when navigating using the `Router.navigate` API, and reading them in the destination component's constructor via `Router.getCurrentNavigation()`.

#### Example:
```ts
// 1. Send state in navigation
...
// 2. Read state in the destination constructor
constructor() {
...
#### Why this feature exists:
- Avoids cluttering the browser URL bar with transient properties (like transactions or order totals).
